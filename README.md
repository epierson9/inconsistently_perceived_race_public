...
1. An alternate to processing from raw data is using the processed data available in the `csv/processed_data` folder. This was created using the script `filter_processed_data.py` by filtering Option 1's output to a reduced set of columns that make anonymizes any PII data, and these csv files can be used to run the analyses.
